...
def rand_rotate_image(input_image: tf.Tensor, input_mask: tf.Tensor):
    ...
    # if a rotation occurs, it will either by 90, 180, or 270
    image = tf.cond(flip_cond, lambda: tf.image.rot90(input_image, k=random.randint(1, 3)), lambda: input_image)
    mask = tf.cond(flip_cond, lambda: tf.image.rot90(input_mask, k=random.randint(1, 3)), lambda: input_mask)
